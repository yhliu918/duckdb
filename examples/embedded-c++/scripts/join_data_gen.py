...
with open('payload.config', 'r') as f:
    ...
    for i in range(len(config)):
        ...
        payload_config.append([int(tmp_config[0]), int(tmp_config[1])])
...
def gen_probe_key(size):
    ...
    print(np.array(heavy_hitters))
    print(f"heavy_hitter_size: {heavy_hitter_size} non_heavy_hitter_size: {non_heavy_hitter_size}")
    non_heavy_hitter_idx = np.random.choice(len(non_heavy_hitter), non_heavy_hitter_size, replace=True)
...
def generate_varchar(max_length,size):
    return np.array([''.join(np.random.choice(list('abcdefghijklmnopqrstuvwxyz'), np.random.randint(0, max_length))) for _ in range(size)])
...
for [type_, size] in payload_config:
...
for [type_, size] in payload_config:
    match type_:
        ...
        case 3:
            payload_tuple_size += size
data_path = '/home/yihao/data_gen/probe{}_build{}_sel{}_skew{}_{}_payload{}_{}'.format(probe_size, build_size, int(selectivity*100), int(heavy_hitter_portion*build_size), int(unique_key_portion*100), payload_col_num, payload_tuple_size)
