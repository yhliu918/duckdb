...
# select A2.k1, B2.k2, B2.v2 from A2, B2, C2 where A2.k1 = B2.k1 and B2.k2 = C2.k2
# |C|<|B|, |C x B| < |A|
...
def gen_data():
    B_k2, C_k2 = gen_k2()
    A_k1, B_k1 = gen_k1()
    B_v2 = gen_random_val(payload_size, B_size)
    
    A = pd.DataFrame({'k1': A_k1, 'rowid': np.arange(0, A_size)})
    B = pd.DataFrame({'k1': B_k1, 'k2': B_k2, 'v2':B_v2,'rowid': np.arange(0, B_size)})
    C = pd.DataFrame({'k2': C_k2, 'rowid': np.arange(0, C_size)})
    
    A.to_parquet('A2.parquet',row_group_size=122880)
    B.to_parquet('B2.parquet',row_group_size=122880)
    C.to_parquet('C2.parquet',row_group_size=122880)

gen_data()

con = duckdb.connect(database = "/home/yihao/duckdb/origin/duckdb/examples/embedded-c++/release/micro_test.db")

for file_name in ['A2', 'B2', 'C2']:
