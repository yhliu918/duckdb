...
std::unordered_set<std::string> parse_plan() {
	...
	std::string directory = path_str + "query/pipeline";
	int pipeline_id = 1;
...
find_materialize_position(std::vector<std::string> attribute, std::unordered_map<std::string, int> &from_pipeline,
	...
	std::ifstream file(path_str + "query/tpcds_schema_10.json");
	file >> schema;
	file.close();
	...
	for (auto &[attr, table_name] : table_info) {
		for (json::iterator it = plan.begin(); it != plan.end(); ++it) {
			if (it.value()["table"] == table_name) {
				...
				while (true) {
					if (pipeline_id == 0 ||
					    !file_exists(path_str + "query/pipeline" + std::to_string(pipeline_id) + ".json")) {
						break;
					}
					int parent_pipeline_id = plan[std::to_string(pipeline_id)]["parent"];
					if (parent_pipeline_id == 0 ||
					    !file_exists(path_str + "query/pipeline" + std::to_string(parent_pipeline_id) + ".json")) {
						break;
					}
...
void write_materialize_config(std::unordered_map<int, std::vector<std::string>> &materialize_config,
                              ...
                              int materialize_strategy_mode = 1, int queue_threshold = 100, int string_length = 0,
                              std::unordered_map<std::string, int> *mat_string_set = nullptr) {
	for (json::iterator it = schema.begin(); it != schema.end(); ++it) {
		std::ofstream file(path_str + "config/table" + it.key(), std::ios::out);
		for (json::iterator it2 = it.value().begin(); it2 != it.value().end(); ++it2) {
			if (std::find(attribute.begin(), attribute.end(), it2.key()) != attribute.end()) {
	...
	for (auto &[pos, attrs] : materialize_config) {
		...
		std::ofstream disable_file(path_str + "config/op_dis_" + std::to_string(mat_op_index), std::ios::out);
		...
		std::ofstream pipeline_file(path_str + "config/pipeline" + std::to_string(pos), std::ios::out);
		bool push_src = false;
		...
		for (auto &[pipeline, attrs] : from_pipeline_to_attr) {
			...
			for (auto &attr : attrs) {
				...
				if (attri_type == 25) {
					...
					pipeline_file << " " << (*mat_string_set)[attr_name];
				}
	...
	for (auto &[pipeid, push_src] : push_source) {
		std::string path = path_str + "config/pipeline" + std::to_string(pipeid);
		if (!file_exists(path)) {
...
int main(int argc, char *argv[]) {
	...
	std::string query_config_path = path_str + "query/";
	if (argc > 3) {
	...
	std::string memory_limit = "100GB";
	if (argc > 10) {
		memory_limit = argv[10];
	}
	int string_length = 0;
	if (argc > 11) {
		string_length = std::stoi(argv[11]);
	}
	...
	std::string query_directory = path_str + "query/pipeline";
	...
	DuckDB db("/home/yihao/duckdb/origin/duckdb/release/tpcds_10_uncom.db");
	Connection con(db);
	...
	// con.Query("SET temp_directory = '/tmp/duckdb_swap';");
	// con.Query("SET max_temp_directory_size = '100GB';");

	DuckDB db_com("/home/yihao/duckdb/origin/duckdb/examples/embedded-c++/release/tpch_small.db");
	...
	int mat_string_length = 0;
	std::unordered_map<std::string, int> materialize_key_set;
	std::unordered_map<std::string, int> mat_string_length_set;
	std::ifstream mat_file_stream(mat_file, std::ios::in);
	while (!mat_file_stream.eof()) {
		mat_file_stream >> attribute >> materialize_pipeline_id >> mat_string_length;
		materialize_key_set[attribute] = materialize_pipeline_id;
		materialize_keys.push_back(attribute);
		mat_string_length_set[attribute] = mat_string_length;
	}
	...
	for (auto &[attribute, materialize_pipeline_id] : materialize_key_set) {
		for (auto &pipeid : possible_mat_options[attribute]) {
			if (pipeid >= materialize_key_set[attribute]) {
	...
	write_materialize_config(materialize_pos, push_source, from_pipeline, materialize_keys, mat_strategy,
	                         materialize_queue_thr, string_length, &mat_string_length_set);
	...
	if (compressed_storage) {
		...
		result->PrintRowNumber();
		std::cout << "{";
		for (auto &[attr, pipe_id] : materialize_key_set) {
			std::cout << "(" << attr << " " << pipe_id << "), ";
		}
		std::cout << "} " << thread << " " << end - start << std::endl;
	} else {
		double start = getNow();
