...
probe_side_load = int(sys.argv[10])
build_side_load = int(sys.argv[11]) 
payload_file = ''
if len(sys.argv)>12:
    payload_file = sys.argv[12] # real-world data file name, if not None, read the payload set from the file
key_set_file = ''
if len(sys.argv)>13:
    key_set_file = sys.argv[13] # real-world data file name, if not None, read the key set from the file
...
if db == 0: #compressed
    ...
    con = duckdb.connect(database = "micro_multi_uncom.db")
    if build_side_load:
