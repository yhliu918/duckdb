...
payload_file = ''
if len(sys.argv)>9:
    payload_file = sys.argv[9] # real-world data file name, if not None, read the payload set from the file
key_set_file = ''
if len(sys.argv)>10:
...
def cal_compression_ratio(file_name):
    con = duckdb.connect(database = "micro_real.db")
    ...
    con2 = duckdb.connect(database = "micro_real_uncom.db")
...
os.system(f'python3 load_db.py 0 {build_size} {probe_size} {sel} {payload_size} {probe_distribution} {build_side_hit_ratio} {build_key_pattern} {probe_key_pattern} {int(generate_probe_side)} {int(generate_build_side)} {payload_file}')
os.system(f'python3 load_db.py 1 {build_size} {probe_size} {sel} {payload_size} {probe_distribution} {build_side_hit_ratio} {build_key_pattern} {probe_key_pattern} {int(generate_probe_side)} {int(generate_build_side)} {payload_file}')

cal_compression_ratio(build_name)
# cal_compression_ratio(probe_name)