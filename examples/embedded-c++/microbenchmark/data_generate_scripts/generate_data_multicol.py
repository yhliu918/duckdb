...
column_numbers = int(sys.argv[9])
payload_file = ''
if len(sys.argv)>10:
    payload_file = sys.argv[10] # real-world data file name, if not None, read the payload set from the file
key_set_file = ''
if len(sys.argv)>11:
    key_set_file = sys.argv[11] # real-world data file name, if not None, read the key set from the file
...
file_path = 'data/'
build_name = f'build_{build_size}_{int(build_side_hit_ratio*100)}_{build_key_pattern}_{payload_size}'
probe_name = f'probe_{build_size}_{probe_size}_{int(sel*10000)}_{probe_key_pattern}_{probe_distribution}_{int(build_side_hit_ratio*100)}'
if key_set_file != '':
    build_name += f'_{key_set_file}'
    probe_name += f'_{key_set_file}'
if payload_file != '':
    build_name += f'_{payload_file}'

build_name+=f"_{column_numbers}"
build_file_name = build_name+ '.parquet'
...
def generate_unique_hit_key(size, non_hit_size, key_set_file=''):
    ...
    key_set = []
    non_hit_key_set = []
    with open('unique_hit_key_set/'+key_set_file+".txt", 'r') as f:
        key_set = [int(line[:-1]) for line in f.readlines()[:size]]

    with open('unique_hit_key_set/'+non_hit_key_file+".txt", 'r') as f:
        ...
        match payload_size:
            ...
            case 8:
                ...
                # currently int64_t file is in binary format
                    ...
                    payload_data = [payload_total[i%len(payload_total)] for i in range(build_size)]
            case _:
                with open('unique_hit_key_set/dbtext/'+payload_file, 'r') as f:
                    payload_total = [(line[:-1]) for line in f.readlines()]
                    payload_data = [payload_total[i%len(payload_total)] for i in range(build_size)]
    return payload_data
        
def cal_compression_ratio(file_name):
    con = duckdb.connect(database = "micro_multi.db")
    res = con.execute(f"select count(distinct block_id) * (select block_size from pragma_database_size()) as num_bytes from pragma_storage_info('{file_name}') group by all;")
   
    df_results = res.fetchdf()
    compressed_size = int(df_results['num_bytes'].iloc[0])
    
    con2 = duckdb.connect(database = "micro_multi_uncom.db")
    res = con2.execute(f"select count(distinct block_id) * (select block_size from pragma_database_size()) as num_bytes from pragma_storage_info('{file_name}') group by all;")
   
    df_results = res.fetchdf()
    uncompressed_size = int(df_results['num_bytes'].iloc[0])
    print(float(compressed_size)/uncompressed_size)
...
#!
if not generate_build_side and not generate_probe_side:
    print(f'{build_file_name} {probe_file_name} Data already exists')
    sys.exit(0)
...
if generate_build_side:
    ...
    build_table = [build_side_key, build_rowid]
    build_column_types = [pa.int32(), pa.int32()]
    build_column_names = ['build_key', 'build_side_rowid']
    fixed_length = True
    if payload_size % 10!=0:
        fixed_length = False
    for i in range(column_numbers):
        build_table.append(generate_payload_data(build_size, payload_size, payload_file,fixed_length))
        match payload_size:
            case 4:
        ...
        build_column_names.append(f'payload_{i}')
...
os.system(f'python3 load_db.py 0 {build_size} {probe_size} {sel} {payload_size} {probe_distribution} {build_side_hit_ratio} {build_key_pattern} {probe_key_pattern} {int(generate_probe_side)} {int(generate_build_side)} {column_numbers} {payload_file}')
os.system(f'python3 load_db.py 1 {build_size} {probe_size} {sel} {payload_size} {probe_distribution} {build_side_hit_ratio} {build_key_pattern} {probe_key_pattern} {int(generate_probe_side)} {int(generate_build_side)} {column_numbers} {payload_file}')
