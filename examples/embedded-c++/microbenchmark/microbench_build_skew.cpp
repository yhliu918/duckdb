...
int main(int argc, char *argv[]) {
	...
	double zipf_factor = atof(argv[13]);
	// if (argc > 13) {
	...
	std::string probe_file_name = "probe_" + std::to_string(build_size) + "_" + std::to_string(probe_size) + "_" +
	                              ...
	                              probe_distribution + "_" + std::to_string(int(build_side_hit_ratio * 100)) + "_" +
	                              std::to_string(int(zipf_factor * 10));
	...
	if (mode == 0) // load from parquet
	{
		...
		con.Query("create table build as from '" + file_path + build_file_name + ".parquet';");
		con.Query("create table probe as from '" + file_path + probe_file_name + ".parquet';");
		...
		// std::ofstream out(
		//     "/home/yihao/duckdb/ht_tmp/duckdb/examples/embedded-c++/release/benchmark_time_0110_dropcache.txt",
		//     std::ios::app);
		// out << mode << " " << thread << " " << probe_size << " " << build_size << " " << selectivity << " "
		//     << payload_size << " " << probe_distribution << " " << build_side_hit_ratio << " " << build_key_pattern
		//     << " " << probe_key_pattern << " " << mat_stat << " " << queue_thr << " " << end - start << std::endl;
	} else if (mode == 1) // load from uncompressed duckdb storage
	{
		DuckDB db("/home/yihao/duckdb/origin/duckdb/examples/embedded-c++/release/micro_zipf.db");
		Connection con(db);
		con.Query("SET threads TO " + thread + ";");
		con.Query("SET disabled_optimizers = 'join_order,build_side_probe_side,COMPRESSED_MATERIALIZATION';");
	...
	{
		DuckDB db("/home/yihao/duckdb/origin/duckdb/examples/embedded-c++/release/micro.db");
		Connection con(db);
		con.Query("SET threads TO " + thread + ";");
		con.Query("SET disabled_optimizers = 'join_order,build_side_probe_side,COMPRESSED_MATERIALIZATION';");
		// std::cout << probe_file_name << " " << build_file_name << std::endl;
